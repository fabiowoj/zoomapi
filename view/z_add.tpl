    ...
    {% if global_tools|length or added_tools|length %}
        <div class="col-sm-3">
            {% if added_tools|length %}
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h2 class="panel-title">{{ 'ToolsAdded'|get_plugin_lang('ZoomPlugin') }}</h2>
                    </div>
                    <ul class="list-group">
                            ...
                            <li class="list-group-item {{ type == tool.id ? 'active' : '' }}">
                                <div class="pull-right">
                                    {% if tool.isActiveDeepLinking %}
                                        <a href="{{ _p.web_plugin }}zoom/start.php?id={{ tool.id }}&{{ _p.web_cid_query }}">
                                            {{ 'settings.png'|img(22, 'Configure'|get_lang) }}
                                        </a>
                                    {% endif %}

                                    <a href="{{ _p.web_plugin }}zoom/configure.php?action=edit&id={{ tool.id }}&{{ _p.web_cid_query }}">
            ...
            {% if global_tools|length %}
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h2 class="panel-title">{{ 'AvailableTools'|get_plugin_lang('ZoomPlugin') }}</h2>
                    </div>
                    <ul class="list-group">
